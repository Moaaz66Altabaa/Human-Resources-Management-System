						...
						<div class="col-md-8 offset-md-2">
                            ...
                            <form method="post" action="{{ route('admin.reports.update' , $report->id) }}">
                                ...
                                <div class="row">
                                    <div class="form-group col-sm-6">
                                        ...
                                        <select name="title" class="select" required>
                                            ...
                                            <option {{ $report->title == 'Salary Report' ? 'selected' : ''}} value="Salary Report">Salary Report</option>
                                            <option {{ $report->title == 'Leave Report' ? 'selected' : ''}} value="Leave Report">Leave Report</option>
                                            <option {{ $report->title == 'Overtime Report' ? 'selected' : ''}} value="Overtime Report">Overtime Report</option>
                                            <option {{ $report->title == 'System Report' ? 'selected' : ''}} value="System Report">System Report</option>
                                            <option {{ $report->title == 'Employees Report' ? 'selected' : ''}} value="Employees Report">Employees Report</option>
                                            <option {{ $report->title == 'Department Report' ? 'selected' : ''}} value="Department Report">Department Report</option>
                                            <option {{ $report->title == 'Others' ? 'selected' : ''}} value="Others">Others</option>
                                        </select>
                                        @error('title')
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="body" rows="4" class="form-control"  required>{{ $report->body }}</textarea>
                                    @error('body')
                                    <p style="color: red"> {{ $message }} </p>
                                    <br>
                                    @enderror
